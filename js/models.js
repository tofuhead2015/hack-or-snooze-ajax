...
class StoryList {
  ...
  async deleteStory(token, storyID) {
    ...
    try {
      console.log('deleteStory ', token, storyID)
      const response = await axios({
        url: `${BASE_URL}/stories/${storyID}`,
        method: "DELETE",
        params: {token: token},
      });  
      this.stories.filter(fav=>fav.storyId !== storyID)    
    } catch (err) {
...
class User {
  ...
  async updateUser(name, password) {
    console.log(name, password, this.username)
    try{
      const response = await axios({
        url: `${BASE_URL}/users/${this.username}`,
        method: "PATCH",
        data: { token: this.loginToken, user: { password: password, name: name } },
      });
      this.name = name
      this.password = password
    } catch (e) {
  ...
  getStory(storyId){
    return this.ownStories.filter(story=>story.storyId === storyId)[0]
  }
  ...
  isFavorite(storyID) {
    console.log("User.isFavorite ", this.favorites, storyID)
